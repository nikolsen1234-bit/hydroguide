...
function costRow(item: CostComparisonItem, isRecommended: boolean): string {
  const cls = isRecommended ? "report-cost-recommended" : "report-cost-muted";
  return `
    <tr class="border-t border-[#c2c6d1]">
      ...
      <td class="p-3 ${cls} report-cost-total border-l border-[#c2c6d1]">${formatNumber(item.toc)} kr</td>
    </tr>`;
}
...
export function buildAiReportPayload(
  ...
  const reserveSourceLabel =
    derivedResults.systemRecommendation.secondarySource === "Ikke beregnet" ? "Ikke beregnet" : activeReserveSource;
  const reserveLogger =
  ...
  return {
    ...
    istilpasning: derivedResults.systemRecommendation.icingAdaptation,
    ...
    ai_on: true,
    rapportutdrag: [
      `Anbefalt hovedløsning: ${recommendation.hovudloysing}.`,
...
<style>
  ...
  :root {
    font-size: var(--hg-type-root-size);
    --hg-type-font-family: "Manrope Variable", "Manrope", system-ui, -apple-system, "Segoe UI", sans-serif;
    --hg-type-root-size: 14.5px;
    --hg-type-page-title-size: 1.9rem;
    --hg-type-section-title-size: 1.688rem;
    --hg-type-category-size: 1rem;
    --hg-type-content-size: 0.935rem;
    --hg-type-ui-size: 0.875rem;
    --hg-type-meta-size: 0.75rem;
    --hg-type-chart-size: 12px;
    --hg-type-weight-medium: 500;
    --hg-type-weight-semibold: 600;
    --hg-type-weight-bold: 700;
    --hg-type-weight-extra: 800;
    --hg-type-content-leading: 1.5rem;
    --hg-type-category-leading: 1.25;
  }
  * { margin: 0; padding: 0; box-sizing: border-box; }
  @page { margin: 0; }
  body { background: #f1f3fc; display: flex; flex-direction: column; align-items: center; padding: 2rem 0; font-family: var(--hg-type-font-family); color: #181c22; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  h1 { font-size: var(--hg-type-section-title-size); font-weight: var(--hg-type-weight-extra); line-height: var(--hg-type-category-leading); }
  h2 { font-size: var(--hg-type-category-size); font-weight: var(--hg-type-weight-bold); line-height: var(--hg-type-category-leading); color: #00305a; margin-bottom: 0.35rem; padding-bottom: 0.25rem; border-bottom: 1px solid #c2c6d1; }
  .meta { font-size: var(--hg-type-meta-size); color: #424750; line-height: var(--hg-type-category-leading); }
  .grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem 2rem; }
  .kv { display: flex; justify-content: space-between; border-bottom: 1px solid #e0e2eb; padding: 0.2rem 0; font-size: var(--hg-type-content-size); line-height: var(--hg-type-content-leading); }
  .kv-label { color: #424750; }
  .kv-value { font-weight: var(--hg-type-weight-medium); }
  ...
  .metric-box .val { font-size: var(--hg-type-category-size); font-weight: var(--hg-type-weight-bold); line-height: var(--hg-type-category-leading); }
  .metric-box .lbl { font-size: var(--hg-type-meta-size); color: #424750; margin-bottom: 0.1rem; line-height: var(--hg-type-category-leading); }
  ...
  .chart-bar-col span { font-size: var(--hg-type-meta-size); color: #424750; }
  table { width: 100%; border-collapse: collapse; font-size: var(--hg-type-content-size); line-height: var(--hg-type-content-leading); text-align: left; }
  table th { background: #ebedf7; padding: 0.25rem 0.5rem; font-weight: var(--hg-type-weight-medium); font-size: var(--hg-type-meta-size); line-height: var(--hg-type-category-leading); }
  table td { padding: 0.25rem 0.5rem; }
  .report-cost-recommended { color: #181c22; font-weight: var(--hg-type-weight-medium); }
  .report-cost-muted { color: #424750; }
  .report-cost-total { font-weight: var(--hg-type-weight-semibold); }
  .section { margin-top: 0.75rem; }
  .footer { margin-top: 0.75rem; padding-top: 0.35rem; border-top: 1px solid #c2c6d1; text-align: center; font-size: var(--hg-type-meta-size); color: #727781; }
  ...
  .toolbar button { font-family: inherit; font-size: var(--hg-type-ui-size); font-weight: var(--hg-type-weight-semibold); line-height: var(--hg-type-category-leading); padding: 0.4rem 0.8rem; border-radius: 0.375rem; background: #00305a; color: white; border: 1px solid #00305a; cursor: pointer; }
  .toolbar button:hover { background: #004780; }
  .begrunnelse { background: #f1f3fc; padding: 0.4rem 0.6rem; border-radius: 0.25rem; font-size: var(--hg-type-content-size); color: #424750; line-height: var(--hg-type-content-leading); }
  .begrunnelse p + p { margin-top: 0.5rem; }
</style>
</head>
<body>
...
<section>
  ...
  <div class="grid-2">
    ...
    <div class="kv"><span class="kv-label">Reservekilde</span><span class="kv-value">${esc(activeReserveSource)}</span></div>
    ...
    <div class="kv"><span class="kv-label">Autonomi</span><span class="kv-value">${formatNumber(derivedResults.systemRecommendation.batteryAutonomyDays)} dager</span></div>
    ...
    <div class="kv"><span class="kv-label">Istilpasning</span><span class="kv-value">${esc(derivedResults.systemRecommendation.icingAdaptation)}</span></div>
  </div>
</section>
    ...
    <div class="chart-bar-wrap">
      ${visibleMonthlyEnergyBalance
        ...
        <div class="chart-bar-col">
                      <span style="font-size:var(--hg-type-meta-size);font-weight:var(--hg-type-weight-medium);">${formatNumber(row.solarProductionKWh, 1)}</span>
...
${selectedRuntimeItem ? `
<section class="section">
  <h2>Reservekilde: driftstid</h2>
  <div class="grid-2">
    <div class="kv"><span class="kv-label">Kilde</span><span class="kv-value">${esc(selectedRuntimeItem.source)}</span></div>
...
<section class="section">
  <h2>Kostnadssammenligning</h2>
  <div style="overflow:hidden;border-radius:0.375rem;border:1px solid #c2c6d1;">
    <table>
      <thead><tr><th>Kilde</th><th style="border-left:1px solid #c2c6d1;">Investering</th><th style="border-left:1px solid #c2c6d1;">Drivstoff/\u00e5r</th><th style="border-left:1px solid #c2c6d1;">Vedlikehold/\u00e5r</th><th style="border-left:1px solid #c2c6d1;">CO\u2082/\u00e5r</th><th style="border-left:1px solid #c2c6d1;">TOC</th></tr></thead>
      <tbody>${derivedResults.costComparison.alternatives.map((item) => costRow(item, item.source === recommendedSource)).join("")}</tbody>
    </table>
  </div>
</section>
<section class="section">
  <h2>${aiRecommendationText ? "KI-underbygging" : "Begrunnelse og tilleggskrav"}</h2>
  ${aiRecommendationText ? `<div class="begrunnelse">${renderAiRecommendationText(aiRecommendationText)}</div>` : `<div class="begrunnelse">
    ${recommendation.grunngiving.length > 0 ? `<p><strong>Begrunnelse:</strong> ${joinSentences(recommendation.grunngiving)}</p>` : ""}
